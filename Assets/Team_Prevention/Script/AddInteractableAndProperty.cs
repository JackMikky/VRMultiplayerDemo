    ...
    public static class AddInteractableAndProperty
    {
        ...
        public static void AddToItemsChildren()
        {
            ...
            foreach (var itemsRoot in itemsRoots)
            {
                ...
                // ItemUseSpawner を Items ルートに追加（未存在時）
                var spawner = itemsRoot.GetComponent<ItemUseSpawner>();
                if (spawner == null)
                {
                    Undo.AddComponent<ItemUseSpawner>(itemsRoot);
                    addedItemUseSpawner++;
                }
            ...
            Debug.Log(
                "[AddInteractableAndProperty] 完了: " +
                $"ItemProperty追加={addedItemProperty}, " +
                $"XRGrabInteractable追加={addedInteractable}, " +
                $"ItemEffectTrigger追加={addedEffectTrigger}, " +
                $"Rigidbody追加={addedRigidbody}, " +
                $"Collider追加={addedCollider}, " +
                $"ItemUseSpawner追加(ItemsRoot)={addedItemUseSpawner}"
            );
        }
    }
}
