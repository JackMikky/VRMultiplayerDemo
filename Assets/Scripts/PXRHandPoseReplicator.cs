...
public class PXRHandPoseReplicator : NetworkBehaviour
{
    /// <summary>
    ...
    /// </summary>
    [Header("Hands and Fingers"), Tooltip("0 is highest, 2 is lowest")] [Range(0, 2), SerializeField]
    int m_FidelityLevel;

    [SerializeField, Tooltip("Determines minimum value threshold for updating finger rotations")]
    float m_MinUpdateDelta = .1f;
    ...
    InputActionProperty[] m_GripInputProperties;
    ...
    NetworkVariable<bool> m_IsInitialized = new NetworkVariable<bool>(false, NetworkVariableReadPermission.Everyone,
        NetworkVariableWritePermission.Owner);

    Pose[] m_handTrackedStartPose = new Pose[2];

    [Header("Offsets")] [SerializeField] Vector3[] m_HandControllerOffsets;
    [SerializeField] Vector3[] m_HandControllerEulerOffsets;
    HandFidelityOption[] m_LocalHandFidelityOptions;

    public XRInputModalityManager.InputMode trackingType
    {
        get => m_TrackingType.Value;
    }

    readonly NetworkVariable<XRInputModalityManager.InputMode> m_TrackingType =
    ...
    private void Awake()
    {
        m_FingerRotationsLeft = new NetworkList<Vector3>(new List<Vector3>(),
            NetworkVariableReadPermission.Everyone, NetworkVariableWritePermission.Owner);
        m_FingerRotationsRight = new NetworkList<Vector3>(new List<Vector3>(),
            NetworkVariableReadPermission.Everyone, NetworkVariableWritePermission.Owner);

        m_FingerCurlLeft = new NetworkList<float>(new List<float>(), NetworkVariableReadPermission.Everyone,
            NetworkVariableWritePermission.Owner);
        m_FingerCurlRight = new NetworkList<float>(new List<float>(), NetworkVariableReadPermission.Everyone,
            NetworkVariableWritePermission.Owner);
    ...
    private void Update()
    {
        if (!m_IsInitialized.Value || !NetworkManager.IsConnectedClient ||
    ...
    void SetupLocalFingerReferences()
    {
        ...
        XRHandSkeletonDriver localLeftHandSkeleton =
            modalityMangager.leftHand.GetComponentInChildren<XRHandSkeletonDriver>();
        XRHandSkeletonDriver localRightHandSkeleton =
            modalityMangager.rightHand.GetComponentInChildren<XRHandSkeletonDriver>();
        ...
        for (int i = 0; i < m_LocalHandFidelityOptions.Length; i++)
        {
            ...
            {
                m_LocalHandFidelityOptions[i].fingerJoints[j].fingerName =
                    m_HandCurler[i].handFidelityOptions[0].fingerJoints[j].fingerName;
                m_LocalHandFidelityOptions[i].fingerJoints[j].jointTransformReferences =
                    new List<JointToTransformReference>();

                // Loop through each joint in the finger and setup the joint references
                for (int k = 0;
                     k < m_HandCurler[i].handFidelityOptions[0].fingerJoints[j].jointTransformReferences.Count;
                     k++)
                {
                    XRHandSkeletonDriver currentHandSkeletonDriver =
                        i % 2 == 0 ? localLeftHandSkeleton : localRightHandSkeleton;
                    ...
                    {
                        if (m_HandCurler[i].handFidelityOptions[0].fingerJoints[j].jointTransformReferences[k]
                                .xrHandJointID == localJoint.xrHandJointID)
                        {
        ...
        for (int i = 0; i < m_LocalHandFidelityOptions[0].fingerJoints.Length; i++)
        {
            for (int j = 0; j < m_LocalHandFidelityOptions[0].fingerJoints[i].jointTransformReferences.Count; j++)
            {
                m_FingerRotationsLeft[currentIdx++] =
                    m_LocalHandFidelityOptions[0].fingerJoints[i].jointTransformReferences[j].jointTransform
                        .eulerAngles;
            }
        }
        ...
        for (int i = 0; i < m_LocalHandFidelityOptions[1].fingerJoints.Length; i++)
        {
            for (int j = 0; j < m_LocalHandFidelityOptions[1].fingerJoints[i].jointTransformReferences.Count; j++)
            {
                m_FingerRotationsRight[currentIdx++] =
                    m_LocalHandFidelityOptions[1].fingerJoints[i].jointTransformReferences[j].jointTransform
                        .eulerAngles;
            }
        }
    }
    ...
    void GetNetworkFingerRotations()
    {
        ...
        {
            for (int j = 0;
                 j < m_HandCurler[hand].handFidelityOptions[0].fingerJoints[i].jointTransformReferences.Count;
                 j++)
            {
                m_HandCurler[hand].handFidelityOptions[0].fingerJoints[i].jointTransformReferences[j].jointTransform
                        .rotation =
                    Quaternion.Slerp(
                        m_HandCurler[hand].handFidelityOptions[0].fingerJoints[i].jointTransformReferences[j]
    ...
    float GetAverageX(int hand, int finger)
    {
        ...
        {
            float currentX = m_LocalHandFidelityOptions[hand].fingerJoints[finger].jointTransformReferences[i]
    ...
    float GetAverageXCombined(int hand)
    {
        ...
        for (int i = startFinger; i < endFinger; i++)
        {
            ...
            {
                float currentX = m_LocalHandFidelityOptions[hand].fingerJoints[i].jointTransformReferences[j]
                    .jointTransform.localEulerAngles.x;
                if (currentX < 0 || currentX > 180)
                {
                    currentX = 0;
    ...
    public void SetTrackingType(XRInputModalityManager.InputMode trackingType)
    {
        ...
        PXRNetworkPlayer.LocalPlayer.SetHandOrigins(m_LeftHandOrigin, m_RightHandOrigin);
    }
