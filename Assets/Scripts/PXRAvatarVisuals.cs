...
[RequireComponent(typeof(PXRNetworkPlayer))]
public class PXRAvatarVisuals : MonoBehaviour
{
    /// <summary>
    /// Head Renderers to change rendering mode for local players.
    /// </summary>
    [Header("Renderer References"), SerializeField,
     Tooltip("Head Renderers to change rendering mode for local players.")]
    ...
    /// </summary>
    protected PXRNetworkPlayer NetworkPlayer;

    public virtual void Awake()
    {
        if (!TryGetComponent(out NetworkPlayer))
        {
            //Utils.LogError(
            //    "XRAvatarVisuals requires a XRINetworkPlayerAvatar component to be attached to the same GameObject. Disabling this component now.");
            enabled = false;
            return;
        }

        //NetworkPlayer.onSpawnedLocal += PlayerSpawnedLocal;
        //NetworkPlayer.onSpawnedAll += PlayerSpawnedAll;
        //NetworkPlayer.onColorUpdated += SetPlayerColor;
    }
    ...
    {
        //if (m_headRend != null)
        //    m_headRend.SetBlendShapeWeight(0, 100 - (NetworkPlayer.playerVoiceAmp * 100));
    }
    ...
    {
        m_HostVisuals.SetActive(m_ShowHostVisuals && NetworkPlayer.NetworkObject.OwnerClientId ==
    ...
    {
        m_HostVisuals.SetActive(m_ShowHostVisuals && NetworkPlayer.NetworkObject.OwnerClientId == newHostId);
    }
}
