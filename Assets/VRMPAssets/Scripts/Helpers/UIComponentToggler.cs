...
namespace XRMultiplayer
{
    ...
    {
        [Header("Component Toggling")] [SerializeField]
        ...
        [SerializeField] CanvasGroup displayCanvasGroup;
        [SerializeField] List<MonoBehaviour> displayComponentToToggle;
#pragma warning disable CS0618 // Type or member is obsolete
        FloatTweenableVariable m_FloatFadeTweenableVariable = new FloatTweenableVariable();
        private FloatTweenableVariable _displayFloatTweenableVariable = new FloatTweenableVariable();
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        void Start()
        {
            ...
            _displayFloatTweenableVariable.Subscribe(UpdateDisplayFade);
        ...
        protected override void Update()
        {
            ...
            {
                float perc =
                    (Mathf.Clamp(currentDistance, m_MinMaxThresholdDistance.x, m_MinMaxThresholdDistance.y) -
                     m_MinMaxThresholdDistance.x) / (m_MinMaxThresholdDistance.y - m_MinMaxThresholdDistance.x);
        ...
        void ToggleFade(bool toggle)
        {
            ...
            m_FadeRoutine = StartCoroutine(m_FloatFadeTweenableVariable.PlaySequence(m_FloatFadeTweenableVariable.Value,
                m_Hidden ? 0.0f : 1.0f, m_FadeDuration, CompleteFade));

            if (_displayFadeRoutine != null) StopCoroutine(_displayFadeRoutine);
            _displayFadeRoutine = StartCoroutine(_displayFloatTweenableVariable.PlaySequence(
                _displayFloatTweenableVariable.Value,
                m_Hidden ? 1.0f : 0.0f, m_FadeDuration, CompleteFade));
        }
        ...
        void UpdateDisplayFade(float fadeAmount)
        {
        ...
        void CompleteFade()
        {
            ...
            if (_displayFloatTweenableVariable.Value >= 1.0f)
            {
                foreach (var item in displayComponentToToggle)
                {
                    if (item != null)
                        item.enabled = true;
                    else
