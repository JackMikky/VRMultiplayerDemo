...
namespace XRMultiplayer
{
    ...
    public class SessionManager : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            {
                Utils.Log(
                    $"{k_DebugPrepend}Distributed Authority request, but no internet connection detected. Falling back to Local Only connection. Please check your network settings.",
                    1);
        ...
        void SetupLocalTransport()
        {
            ...
            if (TryGetComponent(out VoiceChatManager voiceChatManager))
            {
                ...
                Utils.LogWarning(
                    $"{k_DebugPrepend}VoiceChatManager is not supported in {sessionType} sessions. Disabling it.");
            }
        ...
        public async Task<ISession> QuickJoinLobby()
        {
            ...
            try
            {
                QuerySessionsResults results =
                    await MultiplayerService.Instance.QuerySessionsAsync(GetQuickJoinFilterOptions());
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<ISession> CreateSession(string roomName = null, bool isPrivate = false,
            int maxPlayers = XRINetworkGameManager.maxPlayers)
        {
            ...
            try
            {
                ...
                try
                {
                    var options = GetSessionOptions(sessionName, isPrivate, maxPlayers)
                    ...
                    StartCoroutine(PlayConnectionMessage());
                    m_CurrentSession =
                        await MultiplayerService.Instance.CreateOrJoinSessionAsync(sessionId.ToString(), options);
        ...
        SessionOptions GetSessionOptions(string lobbyName, bool isPrivate = false,
            int maxPlayers = XRINetworkGameManager.maxPlayers)
        {
